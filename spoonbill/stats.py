...
class DataPreprocessor:
            ...
            while to_analyze:
                ...
                for key, item in record.items():
                    ...
                    if isinstance(item, dict):
                        ...
                        self.preview_clean_up(self.current_table.name)
                    else:
    ...
    def preview_clean_up(self, table_name):
        """
        Cleaning preview rows of root table after split
        """
        for row in self.tables[table_name].preview_rows:
            remove = [key for key in row.keys() if key not in self.tables[table_name].columns.keys()]
            for key in remove:
                del row[key]
